...
namespace { // private module-only namespace
	...
	public:
		...
		int sendBinary(const std::vector<uint8_t>& message) { return 0;  }
		void sendPing() { }
		void close(bool abruptly) override { }
	...
	public:
		...
		std::vector<std::shared_ptr<WriteBuf>> buffers;
		std::list<std::shared_ptr<WriteBuf>> txbuf;
		std::vector<uint8_t> receivedData;
		FCriticalSection Crit;
		void popFrontBuffer() {
			FScopeLock Lock(&Crit);
			if (txbuf.size() > 0) txbuf.pop_front();
		}
		std::shared_ptr<WriteBuf> getFrontBuffer() {
			FScopeLock Lock(&Crit);
			return txbuf.size() > 0 ? txbuf.front() : std::shared_ptr<WriteBuf>();
		}
		unsigned int txBufferCount() {
			FScopeLock Lock(&Crit);
			return txbuf.size();
		}
		std::shared_ptr<WriteBuf> getBuffer(int size) {
			FScopeLock Lock(&Crit);
			std::shared_ptr<WriteBuf> result;
			for (int i = 0; i < buffers.size(); i++) {
				if (buffers[i]->buffer.capacity() >= size) {
					result = buffers[i];
			...
			if (!result) {
				result = std::shared_ptr<WriteBuf>(new WriteBuf(size));
			}
			return result;
		}
		void releaseBuffer(const std::shared_ptr<WriteBuf> &buffer) {
			FScopeLock Lock(&Crit);
		...
		int sendBuf(const std::shared_ptr<WriteBuf> &buffer) {
			FScopeLock Lock(&Crit); 
			txbuf.push_back(buffer);
			return txbuf.size();
		}
		...
		void poll(int timeout) { // timeout in milliseconds
			...
			if (timeout != 0) {
				...
				FD_ZERO(&wfds);
				FD_SET(sockfd, &rfds);
				if (txBufferCount() > 0) { FD_SET(sockfd, &wfds); }
			...
			std::shared_ptr<WriteBuf> next;
			while (next = getFrontBuffer()) {
				WriteBuf &buf = *next;
				bool cont = true;
				while (buf.offset < buf.length) {
					ret = ::send(sockfd, (char*)&buf.buffer[buf.offset], buf.length-buf.offset, 0);
			...
			if (txBufferCount() == 1 && readyState == CLOSING) {
				closesocket(sockfd);
				readyState = CLOSED;
			}
		}
...
#else
			fcntl(sockfd, F_SETFL, value > O_NONBLOCK : 0);
#endif
		}
		...
		int sendData(wsheader_type::opcode_type type, uint64_t message_size, Iterator message_begin, Iterator message_end) {
			...
			if (readyState == CLOSING || readyState == CLOSED) { return 0; }
			...
			std::shared_ptr<WriteBuf> buffer = getBuffer(header.size() + message_size);
			std::vector<uint8>&txbuf = buffer->buffer;
			txbuf.clear();
			txbuf.insert(txbuf.end(), header.begin(), header.end());
			if (useMask) {
				for (size_t i = 0; i < message_size; i++) {
					txbuf.push_back(*message_begin++ ^ masking_key[i & 0x03]);
				}
			...
			buffer->length = txbuf.size();
			buffer->offset = 0;
			return sendBuf(buffer);
		}

		void close(bool abruptly = false) override {
			...
			if (abruptly) {
				...
				std::shared_ptr<WriteBuf> buffer = getBuffer(header.size());
				std::vector<uint8> &txbuf = buffer->buffer;
